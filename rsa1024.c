...
void rsa1024(char *fileName, int num)
{
	clock_t start_RSA1024, end_RSA1024;
	char *buf;
	char* outputBuf;
	...
	for(k=0; k<num; k++)
	{
		gcry_sexp_build(&params, NULL, "(genkey (rsa (nbits 4:1024)))");  //build 1024 bit RSA keys
		...
		FILE *output = fopen("OutputRSA1024.txt","w");
	    ...
	    end_RSA1024 = clock();
	   	...
	   	t = (double)(end_RSA1024 - start_RSA1024)/CLOCKS_PER_SEC; 
	...
	printf("Mean Encryption and Decryption Time for RSA 1024 %d iterations is: %lf seconds \n",k, meanEnc);
	printf("The Median value for Encryption and Decryption RSA 1024 after %d Iterations is: %lf seconds \n",num, medianEnc);
